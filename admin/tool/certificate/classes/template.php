...
class template {
    ...
    public function save($data) {
        ...
        if (isset($data->cost)) {
            $this->persistent->set('cost', $data->cost);
        }
        if (isset($data->currency)) {
            $this->persistent->set('currency', $data->currency);
        }        
    ...
    public function delete() {
        ...
        $certificaterecord = certificate_data::get_record(['templateid' => $this->persistent->get('id')]); // Shamala.
        
        // Now, finally delete the actual template.
        $this->persistent->delete();
        
        $coursecertificate = new certificate_data($certificaterecord->get('id'));// Shamala.
    ...
    /**
     * Returns the cost of the template.
     *
     * @return string the cost of the template
     */
    public function get_cost() {
        return $this->persistent->get('cost');
    }

    /**
     * Returns the currency of the template.
     *
     * @return string the currency of the template
     */
    public function get_currency() {
        return $this->persistent->get('currency');
    }
    ...
    public static function create($formdata) {
        ...
        $template->cost = $formdata->cost ?? NULL;
        $template->currency = $formdata->currency ?? NULL;
    ...
    public function issue_certificate($userid, $expires = null, array $data = [], $component = 'tool_certificate',
        ...
        $sql = "SELECT * FROM {tool_certificate_issues} WHERE userid = :userid AND templateid=:templateid";
        $checkcertificate = $DB->get_record_sql($sql, ['userid' => $userid, 'templateid' => $issue->templateid ]);
      
        if ($checkcertificate) {       
            if (!empty($courseid)){
                $sqlquery = "SELECT * FROM {tool_certificate_issues} WHERE userid = :userid AND templateid=:templateid AND courseid=:courseid";
                $checkcourse = $DB->get_record_sql($sqlquery, ['userid' => $userid, 'templateid' => $issue->templateid , 'courseid' => $courseid]);
                if ($checkcourse) {
                    $issue->id = $checkcourse->id;
     ...
     * @param \stdClass $issue
     */
    public function send_renewal_notification(\stdClass $issue): void {
        global $DB;

        $user = core_user::get_user($issue->userid);
        $userfullname = fullname($user, true);
        $mycertificatesurl = new moodle_url('/admin/tool/certificate/my.php');
        $subject = get_string('notificationsubjectcertificaterenewal', 'tool_certificate');
        $fullmessage = get_string(
            'notificationmsgcertificaterenewal',
            'tool_certificate',
            ['fullname' => $userfullname, 'url' => $mycertificatesurl->out(false)]
        );
     ...
     * @param \stdClass $issue
     */
    public function update_certificate(\stdClass $issue) {
        global $DB;
        $data = new \stdClass();
        $data->id = $issue->id;     
        $data->templateid = $issue->tid;        
        $issue->timecreated = time();
        $issue->expires = $issue->expirydateabsolute;
        //Update the record into the database.
        $DB->update_record('tool_certificate_issues', $data);
        return $issue->id;
    }
    /**
     ...
     * @return [mixed]
     */
    public static function send_renewalpayment_notification($data) {
        // require_once($CFG->dirroot . '/admin/tool/certificate/lib.php');
        $user = core_user::get_user($data->userid);
        $adminuser = core_user::get_user(2);
        $userfullname = fullname($user, true);
        $mycertificatesurl = new moodle_url('/admin/tool/certificate/my.php');
        $subject = get_string('notificationsubjectcertificaterenewalpayment', 'tool_certificate');
        $fullmessage = get_string(
            'notificationmsgcertificaterenewalpayment',
            'tool_certificate',
            [
                'cost' => $data->cost,
                'fullname' => $userfullname,
                'coursename' => get_course($data->courseid)->fullname,
            ]
        );

        $message = new message();
        $message->courseid = $data->courseid ?? SITEID;
        $message->component = 'tool_certificate';
        $message->name = 'certificaterenewalpayment';
        $message->userfrom = core_user::get_noreply_user();
