...
class certificate {
     ...
     *
     */
    public static function save_update_paymentlogs($userid, $certificateid, $paymethod, $request, $response = '', $logid = 0) {
        $object = new \stdClass();
        $object->userid = $userid;
        $object->certificateid = $certificateid;
        $object->request = $request;
        $object->response = $response;
        $object->paymentmethod = $paymethod;
        $paymentlog = new \tool_certificate\persistent\paymentlogs(0, $object);
        if ($logid == 0) {           
            $paymentlog->create();
        } else {
            $object->id = $logid;          
