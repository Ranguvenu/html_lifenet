...
class details extends dynamic_form {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'cost', get_string('cost', 'tool_certificate'), array('size' => 4));
        $mform->setType('cost', PARAM_RAW);

        $supportedcurrencies = $this->get_currencies();
        $mform->addElement('select', 'currency', get_string('currency', 'tool_certificate'), $supportedcurrencies);
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        if ($template->get_id()) {
            $this->set_data([
                ...
                'cost' => $this->template->get_cost(),
                'currency' => $this->template->get_currency(), ]);
        } else {
