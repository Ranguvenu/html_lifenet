...
function xmldb_tool_certificate_upgrade($oldversion) {
    ...
    if ($oldversion < 2024061103) {
      
        $table = new xmldb_table('tool_certificate_templates');
        $field1 = new xmldb_field('cost');
        $field1->set_attributes(XMLDB_TYPE_CHAR, '20', null, false, null, null);
        if (!$dbman->field_exists($table, $field1)) {
            $dbman->add_field($table, $field1);
        }
        $field2 = new xmldb_field('currency');
        $field2->set_attributes(XMLDB_TYPE_CHAR, '3', null, false, null, null);

        if (!$dbman->field_exists($table, $field2)) {
            $dbman->add_field($table, $field2);
        }
        // Certificate savepoint reached.
        upgrade_plugin_savepoint(true, 2024061103, 'tool', 'certificate');
    }
    if ($oldversion < 2024061106) {
        $table = new xmldb_table('tool_certificate_paymentlogs');
        
        if (!$dbman->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);            
            $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null);
            $table->add_field('certificateid', XMLDB_TYPE_INTEGER, '10', null, null, null);
            $table->add_field('paymentmethod', XMLDB_TYPE_CHAR, '30', null, null, null);
        ...
        upgrade_plugin_savepoint(true, 2024061106, 'tool', 'certificate');
    }
