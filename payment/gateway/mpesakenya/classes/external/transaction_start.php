...
class transaction_start extends external_api {
    ...
    public static function execute_parameters() {
        return new external_function_parameters([
            ...
            'phone' => new external_value(PARAM_RAW, 'The mobile number'),
        ]);
    }
     ...
     * @return array
     */
    public static function execute(string $component, string $paymentarea, int $itemid, string $phone): array {
        global $DB, $USER;
        ...
        while ($code == 409) {
            ...
            $result = $helper->request_payment($random, $reference, $cost, $currency, $phone, $user['country']);
            $code = $result['code'];
            //$code = (int)mpesa_helper::array_helper('code', $resultarr);
        }
        if ($code && $code == 200) {
            $cond = ['paymentid' => $itemid, 'userid' => $userid];
            $DB->delete_records('paygw_mpesakenya', $cond);
            //$transactionid = mpesa_helper::array_helper('xreferenceid', $result) ?? '0';
            $responsebody = $result['responseobj'];
            $data = new \stdClass();
            $data->paymentid = $itemid;
            $data->userid = $userid;
            $data->merchantrequestid = $responsebody->MerchantRequestID;
            $data->checkoutrequestid = $responsebody->CheckoutRequestID;
            $data->moneyid = $helper->token;
            $data->component = $component;
            $data->paymentarea = $paymentarea;
            $data->mobile = $phone;
            $data->timecreated = time();
            $DB->insert_record('paygw_mpesakenya', $data);
        }
        return ['merchantrequestid' => $responsebody->MerchantRequestID, 'checkoutrequestid' => $responsebody->CheckoutRequestID, 'reference' => $reference, 'message' => mpesa_helper::ta_code((int)$code)];
    }
    ...
    public static function execute_returns() {
        return new external_function_parameters([
            'merchantrequestid' => new external_value(PARAM_RAW, 'A valid merchantrequest id'),
            'checkoutrequestid' => new external_value(PARAM_RAW, 'A valid checkoutrequest id'),
