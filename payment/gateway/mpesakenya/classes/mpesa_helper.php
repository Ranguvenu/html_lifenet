...
class mpesa_helper {
     ...
     * @var string The base API URL
     */
    public string $sandboxurl = 'https://sandbox.safaricom.co.ke/';
    ...
    private function get_token() {
        if ($this->token == '') {
            $result = $this->request_post('/oauth/v1/generate?grant_type=client_credentials', [], $this->get_basic_auth(),'GET');
    ...
    public function request_payment(
        ...
        global $CFG, $DB, $USER;
        $allcountries = \paygw_mpesakenya\gateway::get_countries();
        ...
        if (in_array($usercountry, $allcountries)) {
            ...
            $paymentRequest = 'mpesa/stkpush/v1/processrequest';
            ...
            $data = [
                ...
                'CallBackURL' => 'https://lifenet.eabyas.in/payment/gateway/mpesakenya/callback.php',
                'AccountReference' => $transactionid,
                'TransactionDesc' => 'Payment for Leasson'
            ];
           
            $response = $this->guzzle->request('POST', $this->baseurl.$paymentRequest, ['headers' => $headers, 'json' => $data]);
                                 
            $code = $response->getStatusCode();
            $responsebody = $response->getBody()->getContents();          
            $responsebody = json_decode($responsebody);
            // Code for saving custom logs for payment
            $expd = explode('-', $reference);           
            $issue = $DB->get_record('tool_certificate_issues', ['id' => $expd[2]]);
            $certificateid = ($issue->templateid)? $issue->templateid : 0;
            $jsonrequest = json_encode((object)$data);         
            \tool_certificate\certificate::save_update_paymentlogs($USER->id, $certificateid, 'mpesakenya', $jsonrequest, json_encode($responsebody));
 
            $other = array_merge(['verb' => 'Post', 'location' => $paymentRequest, 'answer' => $code], $data);
            $eventargs = ['context' => \context_system::instance(), 'other' => $other];
            $event = \paygw_mpesakenya\event\request_log::create($eventargs);
            $event->trigger();
            return ['code' => $code, 'responsemessage' => $responsebody->ResponseDescription, 'customermessage' => $responsebody->CustomerMessage, 'responseobj' => $responsebody];
     ...
     */
    public function transaction_enquiry(string $checkoutRequestID): array {

        $location = '/mpesa/stkpushquery/v1/query';
        # header for stk push
        $headers = [
            'Authorization' => 'Bearer ' . $this->get_token(),
            'Content-Type' => 'application/json',
        ...
        return $this->request_post($location, $data, $headers, 'POST');
    }
    ...
    private function request_post(
        ...
        try {
            ...
            $eventargs = ['context' => \context_system::instance(), 'other' => $other];
            // Trigger an event.
            \paygw_mpesakenya\event\request_log::create($eventargs)->trigger();
        }
        return $decoded ?? [];
    }
    /**
     ...
     * @return array
     */
    public function certificate_renewal(string $merchantrequestid, int $itemid, string $component, string $area): array {
        global $DB;
        // We assume the transaction pending.
        $returnobject = [];
        $status = 'PENDING';
        $returnobject['status'] =  $status;
        $cond = ['merchantrequestid' => $merchantrequestid, 'paymentid' => $itemid];
        $desc = '';
        $returnobject['desc'] =  $desc;
        if ($rec = $DB->get_record('paygw_mpesakenya', $cond)) {
            $userid = $rec->userid;
            if ($rec->timecompleted == 0) {
                $this->token = $rec->moneyid;
                $result = $this->transaction_enquiry($rec->checkoutrequestid, $this->token);
                ...
                if ($result['ResponseCode'] == '0') {
                    switch ($result['ResultCode']) {
                        ...
                        case '1037': // User cannot be reached
                            ...
                            break;
                        default:
                            //echo "Transaction failed. Reason: " . $status['ResultDesc'];
                            $status = 'FAILED';
                    }
                }
                ...
                if ($status) {
                    if ($status == 'FAILED') {
                        $DB->delete_record('paygw_mpesakenya', $cond);
                    }
                    if ($status == 'SUCCESSFUL') {
                        ...
                        $surcharge = helper::get_gateway_surcharge('mpesakenya');
                        ...
                        if ($currency == $rcurrency && $amount == $ramount) {
                            ...
                            if ($payer[0] == $component && $payer[1] == $area && intval($payer[3]) == $userid) {
                                ...
                                $DB->set_field('paygw_mpesakenya', 'timecompleted', time(), $cond);
                            }
                        } else {
                            // Fraud?
                            $DB->set_field('paygw_mpesakenya', 'area', 'FRAUD');
                            $status = 'FAILED';
                        }
                    }
                }
                    ...
                    if ($status == 'SUCCESSFUL') {
                        ...
                        if ($currency == $rcurrency && $amount == $ramount) {
                            ...
                            if ($payer[0] == $component && $payer[1] == $area && intval($payer[3]) == $userid) {
                                ...
                                helper::deliver_order($component, $area, $itemid, $saved, $userid);
                                $DB->set_field('paygw_mpesakenya', 'timecompleted', time(), $cond);
                                $DB->set_field('paygw_mpesakenya', 'moneyid', $moneyid, $cond);
                            }
                        } else {
                            // Fraud?
                            $DB->set_field('paygw_mpesakenya', 'area', 'FRAUD');
