...
export const getConfigForJs = (component, paymentArea, itemId) => {
    const request = {
        methodname: 'paygw_mpesakenya_get_config_for_js',
        args: {
            component,
 ...
 * @param {string} paymentArea The area of the component that the itemId belongs to
 * @param {number} itemId An internal identifier that is used by the component
 * @param {string} phone An user mobile number
 * @returns {Promise<{transactionid: string, message: string}>}
 */
export const transactionStart = (component, paymentArea, itemId, phone) => {
    const request = {
        methodname: 'paygw_mpesakenya_transaction_start',
        args: {
            component,
            paymentarea: paymentArea,
            itemid: itemId,
