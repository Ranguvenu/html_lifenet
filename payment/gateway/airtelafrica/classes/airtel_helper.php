...
class airtel_helper {
    ...
    public function request_payment(
        ...
        $result = $this->is_testing($userphone) ? $result : $this->request_post($location, $data, $headers);        
        // Code for saving custom logs for payment
        $expd = explode('_', $reference);
        $issue = $DB->get_record('tool_certificate_issues', ['id' => $expd[2]]);
        $jsonrequest = json_encode((object)$data);
        $jsonresponse = json_encode((object)$result);
        $certificateid = ($issue->templateid)? $issue->templateid : 0;
        \tool_certificate\certificate::save_update_paymentlogs($USER->id, $certificateid, 'airtelafrica', $jsonrequest, $jsonresponse);
        return $result;
    }
     ...
     * @return string
     */
    public function certificate_renewal(string $transactionid, int $itemid, string $component, string $area): string {
        global $DB;
