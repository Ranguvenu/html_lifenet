...
function(
    ...
    var handleSendMessage = function(e, data) {
        ...
        var attachments = $('#msgattachments').val().trim();
        var documents = $('#msgdocuments').val().trim();
        ...
        if (text !== '') {
            ...
            var mediaattachments = attachments.split('\\')[2];
            var mediadocuments = documents.split('\\')[2];
            if(typeof mediaattachments !== "undefined"){
                const asize = document.getElementById('msgattachments').files[0].size;
                const afilesize = Math.round((asize / 1024));
                // The size of the file.
                if (afilesize >= 20480) {
                        validated = 0;
                        $('#errormsg').html('<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> File too Big, please select a file less than 20mb');
                    }
                    if(validated){
                        text = mediaattachments;
                    }
                }else
                if(typeof mediadocuments !== "undefined"){
                const dsize = document.getElementById('msgdocuments').files[0].size;        
                const dfilesize = Math.round((dsize / 1024));
                if (dfilesize >= 20480) {
                        validated = 0;
                        $('#errormsg').text('<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> File too Big, please select a file less than 20mb');
                    }
                if(validated){
                    text = mediadocuments;
                }
        ...
        $('#attachmentshow').val('');
        $('#attachmentshow').hide();
        $('[data-region="send-message-txt"]').show();
