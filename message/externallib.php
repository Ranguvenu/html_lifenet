...
class core_message_external extends external_api {
     ...
     * @since Moodle 3.6
     */
    public static function send_messages_to_conversation(int $conversationid, array $messages = [] ,int $attachment) {
        global $CFG, $USER;
        ...
        foreach ($params['messages'] as $message) {
            $createdmessage = \core_message\api::send_message_to_conversation($USER->id, $params['conversationid'], $message['text'],
                $message['textformat'], $params['attachment']);
            $createdmessage->text = message_format_message_text((object) [
                ...
                'fullmessage' => $createdmessage->text,
    ...
    private static function get_conversation_message_structure() {
        return new external_single_structure(
            array(
                ...
                'attachment' => new external_value(PARAM_INT, 'The attachment for the message', VALUE_OPTIONAL),
                'uploadurl' => new external_value(PARAM_RAW, 'The attachment uploadurl', VALUE_OPTIONAL),
                'downloadurl' => new external_value(PARAM_RAW, 'The download url for the attachment', VALUE_OPTIONAL),
            )
        );
    }
