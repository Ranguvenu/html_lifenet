...
class api {
    ...
    public static function get_conversations($userid, $limitfrom = 0, $limitnum = 20, int $type = null,
        ...
        $sql = "SELECT m.id as messageid, mc.id as id, mc.name as conversationname, mc.type as conversationtype, m.useridfrom,
                       ...
                       mc.component, mc.itemtype, mc.itemid, mc.contextid, mca.action as ismuted,m.attachment
        ...
        foreach ($conversations as $conversation) {
            ...
            if($conversation->attachment){
                $fileurl = new \moodle_url($CFG->wwwroot . '/generateimage.php/message/'.$convid.'/'.$conversation->fullmessage);
                $downloadlink = $fileurl . '?forcedownload=1&preview=1&embed=1';
            }else{
                $downloadlink = null;
            }
     ...
     * @param int $attachment the attachment of the message.
    ...
    public static function send_message_to_conversation(int $userid, int $conversationid, string $message,
                                                        int $format, int $attachment = null): \stdClass {
        global $DB, $PAGE, $CFG;
        ...
        $messagerecord = $DB->get_record('messages', ['id' => $messageid], 'id, useridfrom, fullmessage,
                timecreated, fullmessagetrust','attachment');
        $uploadurl = "$CFG->wwwroot/message/upload.php";
        if($messagerecord->attachment || $attachment){
            $fileurl = new \moodle_url($CFG->wwwroot . '/generateimage.php/message/'.$conversationid.'/'.$messagerecord->fullmessage);
            $downloadlink = $fileurl . '?forcedownload=1&preview=1&embed=1';
        }else{
            $downloadlink = null;
        }
        $message = (object) [
            ...
            'attachment' => $attachment ? $attachment : $messagerecord->attachment,
            'uploadurl' => $uploadurl,
