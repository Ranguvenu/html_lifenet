...
function message_format_message_text($message, $forcetexttohtml = false) {
    ...
    if($message->attachment){
        $format = FORMAT_HTML;
        $filename = $message->fullmessage;
        $ext = explode('.', $message->fullmessage);
        $mediamanager = core_media_manager::instance($PAGE);
        $embedoptions = array(
            core_media_manager::OPTION_TRUSTED => true,
            core_media_manager::OPTION_BLOCK => true,
        );
        $messagemediaitem = $DB->get_record('message_media_items', ['messageid' => $message->messageid]);
        if(!empty($messagemediaitem)){
            $mimetype = $messagemediaitem->mimetype;
        }else{
            $imageextenstions = ['jpg','jpeg','gif','png'];
            $videoextenstions = ['mp4'];
            $excelextenstions = ['xls', 'xlsx'];
            $pdfextenstions = ['pdf'];
            if(in_array($ext[1], $imageextenstions)){
                $mimetype = 'image/png';
            }else if(in_array($ext[1], $videoextenstions)) {
                $mimetype = 'video/mp4';
            }else if(in_array($ext[1], $excelextenstions)) {
                $mimetype = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
            }else if(in_array($ext[1], $pdfextenstions)) {
                $mimetype = 'application/pdf';
            }
        }
        $convid = $message->convid ? $message->convid : $message->id;
        $fileurl = new moodle_url($CFG->wwwroot . '/generateimage.php/message/'.$convid.'/'.$message->fullmessage);
        $messagetext = '';
        if (file_mimetype_in_typegroup($mimetype, 'web_image')) {  // It's an image.
            $messagetext .= resourcelib_embed_image($fileurl, $filename);
        }
         else if ($mediamanager->can_embed_url($fileurl, $embedoptions)) {
            // Media (audio/video) file.
            $messagetext .= $mediamanager->embed_url($fileurl, $filename, 0, 0, $embedoptions);
        } else {
            // No other types allowed.
            // We need a way to discover if we are loading remote docs inside an iframe.
            $fileurl->param('embed', 1);
            // Anything else - just try object tag enlarged as much as possible.
            $messagetext .= repolib_embed_general($fileurl, $filename, $mimetype, 0);
        }
    }else if (strval($message->smallmessage) !== '') {
 ...
 * Returns general link or file embedding html.
     ...
     * @return string html
     * @return int forcedownload
     */
    function repolib_embed_general($fullurl, $title, $mimetype, $forcedownload) {
        if ($fullurl instanceof moodle_url) {
            $fullurl = $fullurl->out();
        }
        $fullurl = $fullurl . '?forcedownload='.$forcedownload.'&preview=1&embed=1';
        $param = '<param name="src" value="' . $fullurl . '" />';
        $link = '<div class="generalfile"> '. $title . ' </div>';
        return $link;
    }
