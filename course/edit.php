...
if ($id) {
    ...
    // Fetching certificate template @shamala
    $certificate = certificate_data::get_record(['courseid' => $id]);
    if ($certificate) {
        $course->certificate = $certificate->get('templateid');
    }
...
if ($editform->is_cancelled()) {
    ...
    if (empty($course->id)) {
        ...
        // Saving certificate id in custom table @shamala
        $certificate = new stdClass();
        $certificate->courseid = $course->id;
        $certificate->templateid = $data->certificateid;
        $certificatesave = new \local_custom_courses\persistent\certificate_data(0, $certificate);
        $certificatesave->create();
        ...
        // Update certificate id in custom table @shamala
        $certificate = new stdClass();
        $certificaterecord = certificate_data::get_record(['courseid' => $id]);
        if ($certificaterecord) {
            $certificate->id = $certificaterecord->get('id');
            $certificate->courseid = $course->id;
            $certificate->templateid = ($data->certificateid)? $data->certificateid : null;           
            $certificatesave = new certificate_data(0, $certificate);
            $certificatesave->update();
        } else {
            // Saving certificate id in custom table @shamala
            $certificate = new stdClass();
            $certificate->courseid = $course->id;
