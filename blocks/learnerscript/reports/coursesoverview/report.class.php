...
class report_coursesoverview extends reportbase implements report {
    ...
    public function __construct($report, $reportproperties) {
        ...
        $columns = ['coursename', 'totalactivities', 'completedactivities', 'inprogressactivities', 'grades', 'totaltimespent', 'view'];
    ...
    public function init() {
       if(is_siteadmin($this->userid) && !isset($this->params['filter_users'])){
        ...
        $userid = isset($this->params['filter_users']) && $this->params['filter_users'] > 0 ? $this->params['filter_users'] : $this->userid;
    ...
    public function joins() {
        ...
        if ($this->conditionsenabled) {
            $this->sql .= " JOIN {course_completions} cc ON cc.course = c.id AND cc.userid = ra.userid ";
        }
        parent::joins();
    }
    ...
    public function filters() {
        if ($this->conditionsenabled) {
            ...
            $this->sql .= " AND cc.id IN ($conditions)";
        ...
        switch ($columnname) {
            ...
            case 'inprogressactivities' :
                ...
                $query =  "SELECT COUNT(DISTINCT cm.id) AS inprogressactivities 
                               FROM {course_modules} AS cm
                              WHERE  cm.visible = 1 AND cm.id IN (SELECT coursemoduleid
