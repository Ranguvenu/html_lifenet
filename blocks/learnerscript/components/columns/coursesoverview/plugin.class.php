...
class plugin_coursesoverview extends pluginbase{
	...
	public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0,$reporttype){
        ...
        $inprogressactivityurl = new moodle_url('/blocks/learnerscript/viewreport.php', array('id' => $activityinfoid, 'filter_courses' => $row->id, 'filter_status' => 'inprogress', 'filter_modules' => $this->reportfilterparams['filter_modules'], 'filter_users' => $this->reportfilterparams['filter_users']));
        ...
        switch ($data->column) {
            ...
            case 'inprogressactivities':
                ...
                if (empty($activityinfoid) || empty($reportpermissions)) {
                    ...
                    $row->{$data->column} = !empty($inprogressactivities) ? html_writer::tag('a', $inprogressactivities, array('href' => $inprogressactivityurl)) : '0';
                }
                break;
            case 'completedactivities':
                ...
                if (empty($activityinfoid) || empty($reportpermissions)) {
                    ...
                    $row->{$data->column} = !empty($completedactivities) ? html_writer::tag('a', $completedactivities, array('href' => $completedactivityurl)) : '0';
                }
                break;
            case 'grades':
                if (!isset($row->grades)) {
            ...
            case 'view':
                ...
                $row->{$data->column} = html_writer::tag('a', 'View', array('href' => $url));
        }
