...
function message_send(\core\message\message $eventdata) {
    ...
    if (!$eventdata->notification) {
        ...
        $tabledata->attachment = $eventdata->attachment;
        ...
        if($tabledata->attachment){
            $fullmessageparts = explode('.', $eventdata->fullmessage);
            $filenameparts = pathinfo($eventdata->fullmessage);
            $tabledata->fullmessage = $fullmessageparts[0].'_mid'.$tabledata->id.'.'.$filenameparts['extension'];
            $tabledata->smallmessage = $fullmessageparts[0].'_mid'.$tabledata->id.'.'.$filenameparts['extension'];
            $DB->update_record($table, $tabledata);
        }
